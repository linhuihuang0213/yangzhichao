...
function getTuzki(obj){
    ...
    if(tuzkiNumber){
        ...
        for (var i = 1; i < 41; i++) {
            ...
            str+='<img src="http://'+window.location.host+'/Public/emote/tuzki/t_'+number+'.gif" title="'+alt[i-1]+'" alt="杨志超博客">';
        };
        tuzkiObj.html(str);
        tuzkiNumber=0;
    }else{
        ...
        if(data==1){
            ...
            if(content!='' && content!='请先登录后发表评论'){
                ...
                $.post(ajaxCommentUrl, postData, function(data) {
                    ...
                    if(pid==0){
                        // pid为0表示新增评论
                        var str='<div class="row b-user b-parent"><div class="col-xs-2 col-sm-1 col-md-1 col-lg-1 b-pic-col"><img title="杨志超博客" alt="杨志超博客" src="'+headImg+'" class="b-user-pic"></div><div class="col-xs-10 col-sm-11 col-md-11 col-lg-11 b-content-col"><p class="b-content"><span class="b-user-name">'+nickName+'</span>：'+content+'</p><p class="b-date">'+date+' <a onclick="reply(this)" username="'+nickName+'" pid="'+newPid+'" aid="'+aid+'" href="javascript:;">回复</a></p><div class="b-clear-float"></div></div></div>';
                        $('.b-user-comment').prepend(str);
                    }else{
                        // pid不为0表示是回复评论
                        var str='<div class="row b-user b-child"><div class="col-xs-2 col-sm-1 col-md-1 col-lg-1 b-pic-col"><img title="杨志超博客" alt="杨志超博客" src="'+headImg+'" class="b-user-pic"></div><ul class="col-xs-10 col-sm-11 col-md-11 col-lg-11 b-content-col"><li class="b-content"><span class="b-reply-name">'+nickName+'</span><span class="b-reply">回复</span><span class="b-user-name">'+replyName+'</span>：'+content+'</li><li class="b-date">'+date+' <a onclick="reply(this)" pid="'+newPid+'" aid="'+aid+'" username="'+replyName+'" href="javascript:;">回复</a></li><li class="b-clear-float"></li></ul></div>';
